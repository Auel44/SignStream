...
resource "aws_lambda_function" "ws_connect" {
  ...
  package_type  = "Image"
  image_uri     = "${data.aws_ssm_parameter.ecr_ws_connect.value}:${var.image_tag}"
  ...
  depends_on = [
    ...
    aws_iam_role_policy.ws_connect,
    aws_iam_role_policy_attachment.ws_connect_basic,
  ]
}

resource "aws_lambda_permission" "apigw_invokes_ws_connect" {
