...
# Absolute path to this repo's assets/ directory.
#
# The original code used "./assets/...". Blender resolves relative paths
# in bpy.data.libraries.load() against the open .blend file rather than the
# working directory, and in --background mode there is none — so the path
# collapsed to the drive root (C:ssets\defaults.blend). Anchoring to
# __file__ makes it correct regardless of where the command is run from.
...
def execute_pipeline(arguments: argparse.Namespace) -> None:
    ...
    if arguments.render_sentence:
        glue = Glue(
            mms=mms,
            ignore_bone_list=str(_ASSETS / "ignorelist.json"),
            src_blendfile=str(_ASSETS / "defaults.blend"),
    ...
    config_path = _ASSETS / "controller_config.json"
